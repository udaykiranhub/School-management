    ...
    const fetchStrengthData = async (classList) => {
        try {
            ...
            const strengthPromises = classList.map(async (cls) => {
                ...
                const classTotals = sectionsWithStudents.reduce((acc, { students }) => {
                    const transport = students.filter(s => s.transport).length;
                    const hostel = students.filter(s => s.hostel).length;
                    const others = students.filter(s => !s.transport && !s.hostel).length;
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-2xl font-bold text-gray-800 mb-6">Strength Report</h1>

                <div className="mb-6">
                    <div className="flex gap-4">
                        <button
                            onClick={() => handleViewTypeChange('class')}
                            className={`px-4 py-2 rounded-lg ${viewType === 'class'
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg ${viewType === 'section'
                                    ? 'bg-blue-600 text-white'
                                    : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                }`}
                        >
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                                ...
                                {strengthData.map((data, index) => (
                                    viewType === 'class' ? (
                                        ...
                                        data.sections.map((sectionData, sIndex) => (
                                            <tr key={sectionData.section._id} className="hover:bg-gray-50">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {sIndex + 1}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {sectionData.students.filter(s => s.transport).length}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {sectionData.students.filter(s => s.hostel).length}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {sectionData.students.filter(s => !s.transport && !s.hostel).length}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            <tfoot className="bg-gray-50">
                                <tr>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {strengthData.reduce((acc, data) => acc + (viewType === 'class' ? data.transport :
                                            data.sections.reduce((secAcc, sec) => secAcc + sec.students.filter(s => s.transport).length, 0)), 0)}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {strengthData.reduce((acc, data) => acc + (viewType === 'class' ? data.hostel :
                                            data.sections.reduce((secAcc, sec) => secAcc + sec.students.filter(s => s.hostel).length, 0)), 0)}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {strengthData.reduce((acc, data) => acc + (viewType === 'class' ? data.others :
                                            data.sections.reduce((secAcc, sec) => secAcc + sec.students.filter(s => !s.transport && !s.hostel).length, 0)), 0)}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {strengthData.reduce((acc, data) => acc + (viewType === 'class' ? data.total :
                                            data.sections.reduce((secAcc, sec) => secAcc + sec.students.length, 0)), 0)}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
